...
void test_global() {    
    ...
    int send_sock = socket(AF_INET6, SOCK_DGRAM, SOCK_NONBLOCK);
    CU_ASSERT(send_sock > 0);
    CU_ASSERT(bind(send_sock, &address, sizeof(struct sockaddr_in6)) == 0);
    ...
    stream_t rx_to_hd;
    CU_ASSERT(allocate_stream(&rx_to_hd, 2048*2048) == 0);
    
    stream_t hd_to_rx;
    CU_ASSERT(allocate_stream(&hd_to_rx, 2048*2048) == 0);
    bool wait = false;
    ...
    client_t client;
    CU_ASSERT(initialize_client(&client, 0, "./bin/%d", &address, sizeof(address)) == 0);
    ...
    *decoded = allocate_packet();
    ...
    pkt4.type = DATA;
    pkt1.timestamp = 10101010101010101;
    pkt4.length = 0;
    pkt4.seqnum = 2;
    ...
    CU_ASSERT(received.timestamp == 99999999999);
    CU_ASSERT(received.truncated == false);
    CU_ASSERT(received.length == 0);
    CU_ASSERT(received.window == 31);
    ...
    CU_ASSERT(received.timestamp == 10101010101010101);
    CU_ASSERT(received.truncated == false);
    CU_ASSERT(received.length == 0);
    ...
    CU_ASSERT(strcmp(string, "ZA WAAAARUUUUDOOOOOOOOO!!!!!!\n"));
    fgets(string, sizeof(string), fd);
